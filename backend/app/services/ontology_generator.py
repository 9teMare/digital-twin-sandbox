...
- 具体的平台用户、活动创建者、内容创作者、参与者、客户、交易者等可在系统或社媒中行动/发声的账号主体
- 公司、企业（包括其官方账号）
- 组织机构（大学、协会、NGO、工会等）
- 政府部门、监管机构
- 媒体机构（报纸、电视台、自媒体、网站）
...
- 主题/话题（如"学术诚信"、"教育改革"）
- 观点/态度（如"支持方"、"反对方"）
- 时间戳、日期、版本号、指标数值、字段名、枚举值、ID、参数、配置项（如 `uid_hash`、`bd_name`、`2026-06`）
- 用户属性本身（如"vip_code"、"language"、"risk_level"）或表单字段本身
- 活动步骤、业务流程、操作事件（如注册、充值、交易、提交任务、领取奖励），除非文本明确把它描述成一个可发声/可行动的主体或群体
...
- 人物/账号主体必须归入 `Person` 或更具体的人物类型（如 `CampaignCreator`、`User`、`Participant`、`Customer`），不要让人物主体退化成泛泛的 `Entity`
- 字段、时间、指标和活动步骤应该作为属性、关系、事实或上下文处理，不要设计成 entity type
...
6. 人物/账号主体必须使用 Person 或具体人物类型；时间戳、字段名、用户属性、注册/充值/交易等活动步骤不能作为实体类型
"""
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            '"""',
            '自定义实体类型定义',
            '由Digital Twin Agent Sandbox自动生成，用于社会舆论模拟',
            '"""',
            '',
